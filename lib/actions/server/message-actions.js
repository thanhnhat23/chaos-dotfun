...
class ActionsMessageActions
{
    ...
    preparePlayerCurrentAction(playerSchema, data)
    {
        ...
        if(!runAction || (!playerAction && !classPathSkill)){
            // Fallbacks: try common default keys if client didn't send a valid one.
            let candidates = [];
            if(runAction){ candidates.push(runAction); }
            // some default commons
            candidates.push('physical_attack');
            candidates.push('attack');
            candidates.push('attackShort');
            let resolved = false;
            for(let key of candidates){
                if(!key){ continue; }
                playerAction = sc.get(playerSchema.actions, key, false);
                classPathSkill = sc.get(playerSchema.skillsServer.classPath.currentSkills, key, false);
                if(playerAction || classPathSkill){
                    runAction = key;
                    resolved = true;
                    break;
                }
