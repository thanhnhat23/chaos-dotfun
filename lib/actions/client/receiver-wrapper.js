...
class ReceiverWrapper extends Receiver
{
    ...
    onInitClassPathEnd(message)
    {
        ...
        this.updateDefaultActionKey(message);
    }
    ...
    updateDefaultActionKey(message)
    {
        let resolvedKey = this.resolveDefaultActionKey(message);
        if(!resolvedKey){
            return;
        }
        this.gameManager.config.client.ui = this.gameManager.config.client.ui || {};
        this.gameManager.config.client.ui.controls = this.gameManager.config.client.ui.controls || {};
        this.gameManager.config.client.ui.controls.defaultActionKey = resolvedKey;
        let currentPlayer = this.gameManager.getCurrentPlayer();
        if(currentPlayer){
            currentPlayer.defaultActionKeyConfig = resolvedKey;
        }
        this.events?.emitSync('reldens.defaultActionKeyUpdated', resolvedKey, this.gameManager);
    }

    resolveDefaultActionKey(message)
    {
        let configuredKey = this.gameManager.config.get('client/ui/controls/defaultActionKey');
        let skillsMap = sc.getByPath(message, ['data', ActionsConst.MESSAGE.DATA.SKILL_LEVEL], {});
        let availableSkills = Object.keys(skillsMap || {});
        let lastAttackKey = sc.getByPath(message, ['data', ActionsConst.MESSAGE.DATA.LAST_ATTACK_KEY], '');
        if(lastAttackKey && availableSkills.includes(lastAttackKey)){
            return lastAttackKey;
        }
        if(configuredKey && availableSkills.includes(configuredKey)){
            return configuredKey;
        }
        if(availableSkills.length){
            return availableSkills[0];
        }
        return configuredKey || 'physical_attack';
    }
