...
class HomepageLoader
{
    ...
    static async loadContents(requestLanguage, distPath, initialConfiguration)
    {
        ...
        let configScriptContents = '<script type="text/javascript" id="chaos-initial-config">'
            + 'window.chaosInitialConfig = ' + JSON.stringify(initialConfiguration) + ';'
            + 'window.reldensInitialConfig = window.chaosInitialConfig;'
            + '</script>';
        if(!html){
        ...
        html = html.replace(
            '<script type="text/javascript" id="chaos-initial-config"></script>',
