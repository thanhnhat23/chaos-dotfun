...
const { StreamersRepository } = require('./streamers-repository');
const { StreamersApi } = require('./streamers-api');
...
class ServerManager
{
    ...
    usersManager = false;
    translations = {};
    streamersRepository = null;
    streamersApiRegistered = false;
    ...
    enableServeStaticsAndHomePage()
    {
        ...
        this.registerStreamersApi();
    }


    registerStreamersApi()
    {
        if(this.streamersApiRegistered){
            return;
        }
        if(!this.app){
            Logger.warning('[Chaos] Streamers API skipped - app instance unavailable.');
            return;
        }
        const storagePath = path.join(this.projectRoot, 'storage', 'chaos-streamers.json');
        const ttlMs = Number(process.env.CHAOS_STREAMERS_TTL_MS || 300000);
        this.streamersRepository = new StreamersRepository({ storagePath, ttlMs });
        StreamersApi.register({ app: this.app, repository: this.streamersRepository, ttlMs });
        this.streamersApiRegistered = true;
        Logger.info('[Chaos] Streamers API attached.');
    }
