...
class LoginManager
{
    ...
    constructor(props)
    {
        ...
        this.expectedClientVersion = this.getExpectedClientVersion();
    }
    ...
    getExpectedClientVersion()
    {
        let configured = this.config.getWithoutLogs('client/gameEngine/version', false);
    ...
    isClientVersionValid(clientVersion)
    {
        this.expectedClientVersion = this.getExpectedClientVersion();
        if('' === this.expectedClientVersion){
            return true;
        }
        if(!clientVersion){
            return false;
        }
        Logger.debug('Version check', {clientVersion: String(clientVersion), expected: String(this.expectedClientVersion)});
        return String(clientVersion) === String(this.expectedClientVersion);
    }
    ...
    async processUserRequest(userData = false)
    {
        ...
        let receivedVersion = sc.get(userData, 'clientVersion', false);
        if(!this.isClientVersionValid(receivedVersion)){
            let versionResult = {error: 'Client version mismatch, please reload.'};
            Logger.warning('Client version mismatch.', {received: receivedVersion, expected: this.expectedClientVersion});
            this.events.emitSync('reldens.clientVersionMismatch', this, userData, versionResult);
