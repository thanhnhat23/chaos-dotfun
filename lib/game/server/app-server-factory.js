...
class AppServerFactory
{
    ...
    enableServeHome(app, distPath, initialConfiguration)
    {
        ...
        const spaRoutes = ['/', '/streamer', '/viewer', '/guest'];
        spaRoutes.forEach((route) => {
            app.get(route, limiter, serveHome);
        });
        app.get('*', async (req, res, next) => {
            ...
            if (pathName.startsWith('/api/') || pathName.startsWith('/colyseus') || pathName.includes('.')) {
                return next();
            }
