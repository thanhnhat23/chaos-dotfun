...
class SceneDynamic extends Scene {
    ...
    constructor(key, data, gameManager) {
        ...
        this.lastAutoAttackTargetId = null;
        this.lastAutoAttackTime = 0;
        this.autoAttackCooldownMs = null;
    ...
    update(time, delta) {
        ...
        this.autoFocusNearestEnemyIfNeeded();
            ...
            if (!this.player.currentTarget) {
                ...
                for (let k of Object.keys(objects)) {
                    ...
                    if (!nearest || dist < nearest.dist) {
                        nearest = { id: obj.key, type: ObjectsConst.TYPE_OBJECT, dist, obj };
                ...
                if (nearest) {
                    ...
                    this.approachAndAttack(nearest.obj);
                    return;
                }
            }
    ...
    // Approach target object to a reasonable range before sending attack.
    approachAndAttack(targetObj) {
        if (!targetObj) {
            return this.player.runActions();
        }
        let currentPlayer = this.gameManager.getCurrentPlayer();
        if (!currentPlayer || 'function' !== typeof currentPlayer.getPosition) {
            return this.player.runActions();
        }
        let pos = currentPlayer.getPosition();
        let dx = targetObj.x - pos.x;
        let dy = targetObj.y - pos.y;
        let dist = Math.hypot(dx, dy);
        let range = Number(this.gameManager.config.getWithoutLogs('client/actions/autoAttackRange', 60));
        if (dist > range) {
            let ratio = (dist - range) / dist;
            let tx = pos.x + dx * ratio + this.player.leftOff;
            let ty = pos.y + dy * ratio + this.player.topOff;
            let fp = { worldX: tx, worldY: ty };
            if (this.appendRowAndColumn(fp)) {
                this.player.moveToPointer(fp);
            }
        }
        this.player.runActions();
    }
    ...
    // Automatically focus nearest enemy within range when no target is selected.
    autoFocusNearestEnemyIfNeeded() {
        // feature flag:
        let enabled = this.configManager.getWithoutLogs('client/actions/autoFocus/enabled', true);
        if (!enabled) {
            return;
        }
        if (!this.player || this.player.isDeath() || this.player.isDisabled()) {
            this.resetAutoTargetingState();
            return;
        }
        let currentPlayer = this.gameManager.getCurrentPlayer();
        if (!currentPlayer || 'function' !== typeof currentPlayer.getPosition) {
        ...
        let currentTargetId = this.player.currentTarget?.id || null;
        let isFollowingAutoTarget = currentTargetId && currentTargetId === this.lastAutoAttackTargetId;
        // Do not override manual target:
        if (this.player.currentTarget && !isFollowingAutoTarget) {
            return;
        }
        let range = Number(this.configManager.getWithoutLogs('client/actions/autoFocus/range', 120));
        let nearest = this.getNearestEnemyWithin(range);
        if (!nearest) {
            this.resetAutoTargetingState();
            return;
        }
        let previous = this.player.currentTarget ? Object.assign({}, this.player.currentTarget) : false;
        let isNewTarget = !previous || previous.id !== nearest.id;
        if (isNewTarget) {
            this.player.currentTarget = { id: nearest.id, type: ObjectsConst.TYPE_OBJECT };
            this.gameManager.gameEngine.showTarget(nearest.targetName || '', this.player.currentTarget, previous);
        }
        this.triggerAutoAttack(nearest, isNewTarget);
    }

    getNearestEnemyWithin(range) {
        let currentPlayer = this.gameManager.getCurrentPlayer();
        if (!currentPlayer || 'function' !== typeof currentPlayer.getPosition) {
            return null;
        }
        let pos = currentPlayer.getPosition();
        let nearest = null;
        for (let key of Object.keys(this.objectsAnimations || {})) {
            let obj = this.objectsAnimations[key];
            if (obj.type !== ObjectsConst.TYPE_ENEMY) {
                continue;
            }
            ...
            if (!nearest || dist < nearest.dist) {
                nearest = { id: obj.key, dist, targetName: obj.targetName, obj };
            }
        }
        return nearest;
    }
    ...
    ensureAutoAttackConfig() {
        if (null === this.autoAttackCooldownMs) {
            this.autoAttackCooldownMs = Number(
                this.configManager.getWithoutLogs('client/actions/autoFocus/attackInterval', 700)
            );
        }
    }

    triggerAutoAttack(nearest, force = false) {
        if (!nearest?.obj) {
            return;
        }
        this.ensureAutoAttackConfig();
        let now = Date.now();
        let shouldAttack = force
            || nearest.id !== this.lastAutoAttackTargetId
            || now - this.lastAutoAttackTime >= this.autoAttackCooldownMs;
        if (!shouldAttack) {
            return;
        }
        this.lastAutoAttackTargetId = nearest.id;
        this.lastAutoAttackTime = now;
        this.approachAndAttack(nearest.obj);
    }

    resetAutoTargetingState() {
        this.lastAutoAttackTargetId = null;
        this.lastAutoAttackTime = 0;
    }
