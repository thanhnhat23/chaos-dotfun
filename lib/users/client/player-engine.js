...
class PlayerEngine
{
    ...
    runActions()
    {
        let actionKey = this.resolveActionKey();
    ...
    resolveActionKey()
    {
        let configuredKey = this.defaultActionKeyConfig || this.config.get('client/ui/controls/defaultActionKey');
        let availableSkills = Object.keys(sc.get(this.gameManager, 'skills.skills', {}) || {});
        if (availableSkills.length) {
            if (!configuredKey || !availableSkills.includes(configuredKey)) {
                configuredKey = availableSkills[0];
                this.defaultActionKeyConfig = configuredKey;
                this.ensureConfigDefaultAction(configuredKey);
            }
        }
        if (!configuredKey) {
            configuredKey = 'physical_attack';
        }
        return configuredKey;
    }

    ensureConfigDefaultAction(actionKey)
    {
        if (!actionKey) {
            return;
        }
        ...
        this.gameManager.config.client.ui.controls.defaultActionKey = actionKey;
    }
